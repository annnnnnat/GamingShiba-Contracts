...
contract GamingShibaContract is Context, IERC20, Ownable, Initializable {
        ...
        returns (FeeTier memory)
    {
        uint256 _fees = _tier
            ...
            .add(_tier.communityFee)
            .add(_tier.taxFee)
            .add(_tier.betweenWalletsFee);
    ...
    function checkFeesChanged(
        ...
        uint256 _feesFinal = _fees.sub(_oldFee).add(_newFee);
        ...
        returns (FeeValues memory)
    {
        ...
        uint256 tTransferFee = tValues
            .tStaking
            .add(tValues.tDev)
            .add(tValues.tMarketing)
            .add(tValues.tCommunity)
            .add(tValues.tBetweenWallets);
        ...
        returns (tFeeValues memory)
    {
        ...
        uint256 temp = tAmount.sub(tValues.tStaking).sub(tValues.tDev).sub(
            tValues.tMarketing
        );
        tValues.tTransferAmount = temp
            .sub(tValues.tCommunity)
            .sub(tValues.tBetweenWallets)
            .sub(tValues.tFee);
        return tValues;
    }
